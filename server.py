...
def verify_password(plain_password, hashed_password):
    # Хэшируем через SHA-256 перед bcrypt (обход лимита 72 байта)
    hashed = hashlib.sha256(plain_password.encode()).hexdigest()
    return pwd_context.verify(hashed, hashed_password)

def get_password_hash(password):
    # Хэшируем через SHA-256 перед bcrypt
...
@app.get("/works")
def get_works(status: str = "published", current_user: dict = Depends(get_current_user)):
    db = get_db()
    if current_user["role"] == "writer":
        if status == "draft":
            rows = db.execute("""
                SELECT w.id, w.title, u.username as author, w.status, w.created_at 
                FROM works w JOIN users u ON w.author_id = u.id 
                WHERE w.author_id = ? AND w.status = 'draft' ORDER BY w.created_at DESC
            """, (current_user["id"],)).fetchall()
        else:
            rows = db.execute("""
                SELECT w.id, w.title, u.username as author, w.status, w.created_at 
                FROM works w JOIN users u ON w.author_id = u.id 
                WHERE w.author_id = ? OR w.status = 'published' ORDER BY w.created_at DESC
            """, (current_user["id"],)).fetchall()
    else:
        rows = db.execute("""
            SELECT w.id, w.title, u.username as author, w.status, w.created_at 
...
def get_work(work_id: int, current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] == "reader" and row["status"] != "published":
...
def create_work(work: WorkCreate, current_user: dict = Depends(get_current_user)):
    ...
    db = get_db()
    cursor = db.execute("INSERT INTO works (title, content, author_id, status, created_at) VALUES (?, ?, ?, ?, ?)",
                        (work.title, work.content, current_user["id"], work.status, datetime.now().strftime("%d.%m.%Y %H:%M")))
    db.commit()
    db.close()
    return {"id": cursor.lastrowid}

@app.put("/works/{work_id}")
def update_work(work_id: int, work: WorkUpdate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    row = db.execute("SELECT * FROM works WHERE id = ?", (work_id,)).fetchone()
    if not row or row["author_id"] != current_user["id"]:
        db.close()
        raise HTTPException(status_code=403, detail="Not your work")
    updates, params = [], []
    if work.title: updates.append("title = ?"); params.append(work.title)
    if work.content: updates.append("content = ?"); params.append(work.content)
    if work.status: updates.append("status = ?"); params.append(work.status)
    if updates:
...
def get_feedback(work_id: int, current_user: dict = Depends(get_current_user)):
    db = get_db()
    rows = db.execute("SELECT f.category, f.text, u.username as reviewer, f.created_at FROM feedback f JOIN users u ON f.user_id = u.id WHERE f.work_id = ? ORDER BY f.created_at DESC", (work_id,)).fetchall()
    db.close()
    return [dict(r) for r in rows]
