...
class AuthWindow(QMainWindow):
    ...
    def login(self):
        ...
        if not username or not password:
            ...
            return
        try:
            # Отправляем только логин и пароль (роль сервер берет из БД при входе)
            ...
            if resp.status_code == 200:
                ...
                # Важно: роль выбирается пользователем при входе для интерфейса
                ...
                detail = resp.json().get('detail', 'Неверные данные')
                self.status_label.setText(f"Ошибка: {detail}")
...
class MainWindow(QMainWindow):
    ...
    def load_work(self, item):
        ...
        try:
            ...
            self.menu.setCurrentRow(2)  # Переход на вкладку отзывов
    ...
    def publish_work(self):
        ...
        if not text.strip():
            ...
            return
        try:
            resp = requests.post(f"{SERVER_URL}/works", json={"title": title.strip(), "content": text},
                                 headers=self.get_headers(), timeout=10)
            resp.raise_for_status()
    ...
    def refresh_feedback_list(self):
        ...
        # ИСПРАВЛЕНО: корректное обращение к self.feedback_data
