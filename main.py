...
SERVER_URL = "https://litpotokserver-1.onrender.com"
...
class MainWindow(QMainWindow):
    def __init__(self, username, role, token):
        ...
        self.setup_library_page()  # Индекс 0

        if self.role == "writer":
            self.setup_drafts_page()  # Индекс 1 (только для писателя)

        self.setup_editor_page()  # Индекс 1 (читатель) или 2 (писатель)
        self.setup_feedback_page()  # Индекс 2 (читатель) или 3 (писатель)
        self.setup_settings_page()  # Индекс 3 (читатель) или 4 (писатель)
    ...
    def switch_page(self, index):
        # Просто переключаем на тот же индекс - страницы добавлены по порядку!
    ...
    def setup_drafts_page(self):
        """Страница 1 (только писатель): Черновики"""
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(15, 15, 15, 15)
    ...
    def setup_editor_page(self):
        """Страница: Редактор (2 для писателя, 1 для читателя)"""
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(15, 15, 15, 15)

        # Кнопка "Оставить отзыв" для читателей (справа сверху)
    ...
    def setup_feedback_page(self):
        """Страница: Отзывы (3 для писателя, 2 для читателя)"""
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(15, 15, 15, 15)
        ...
        # Форма добавления отзыва (только для читателей)
        ...
        if self.role == "writer":
            # Писатели только смотрят отзывы
    ...
    def setup_settings_page(self):
        """Страница: Настройки (4 для писателя, 3 для читателя)"""
        page = QWidget()
        layout = QVBoxLayout(page)
    ...
    def refresh_drafts(self):
        """Обновить список черновиков (только писатель)"""
    ...
    def load_work(self, item):
        """Загрузить произведение в редактор"""
        ...
        try:
            ...
            # Кнопка отзыва для читателя
            if self.role == "reader" and hasattr(self, 'review_btn'):
                self.review_btn.setVisible(True)

            self.load_feedback(work_id)

            # Переход на вкладку редактора
            if self.role == "writer":
                self.menu.setCurrentRow(2)  # Редактор
            else:
                self.menu.setCurrentRow(1)  # Чтение
    ...
    def open_feedback_modal(self):
        """Открыть окно для отзыва (читатель)"""
    ...
    def publish_work(self):
        """Опубликовать произведение (писатель)"""
        if self.role != "writer": return
