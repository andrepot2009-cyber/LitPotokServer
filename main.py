...
class MainWindow(QMainWindow):
    def __init__(self, username, role, token):
        ...
        self.feedback_data = []
        self.library_mode = "published"  # "published" или "draft"
        ...
        menu_items = ["Библиотека", "Редактор", "Отзывы", "Настройки"]
        if self.role == "writer":
            menu_items.insert(1, "Черновики")
    ...
    def switch_page(self, index):
        if self.role == "writer":
            if index == 0:
                self.library_mode = "published"
                self.refresh_library()
            elif index == 1:
                self.library_mode = "draft"
                self.refresh_drafts()
            else:
                self.content_area.setCurrentIndex(index - 1 if index > 1 else index)
        else:
    ...
    def setup_settings_page(self):
        ...
        profile_label = QLabel(
            f"Профиль: {self.username} | Роль: {'Читатель' if self.role == 'reader' else 'Писатель'}")
                ...
                try:
                    resp = requests.post(f"{SERVER_URL}/feedback",
                                         json={"work_id": self.current_work_id, "category": category,
                                               "text": text.strip()}, headers=self.get_headers(), timeout=10)
                    resp.raise_for_status()
        ...
        try:
            resp = requests.post(f"{SERVER_URL}/feedback",
                                 json={"work_id": self.current_work_id, "category": self.category_combo.currentText(),
                                       "text": text}, headers=self.get_headers(), timeout=10)
            resp.raise_for_status()
